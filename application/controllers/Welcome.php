...
class Welcome extends CI_Controller {
	...
	public function do_login(){  
		...
		->get_where("users",$arr)->row();
		if(!empty($row)){
			if ($this->bcrypt->check_password($post['password'], $row->password)){
				if($row->user_status == 1){
					$this->config_model->update_token($row->user_id, $this->security->get_csrf_hash(), $post["fcm_token"]);
					$data = array(
							"user_id" 		=> $row->user_id
...
*/
	...
	function get_card_empty(){
		$cards_amount = $this->db->where("card_amount", 0)->get("card")->result();
		if(count($cards_amount) > 0){
			$res = "هناك بطاقات عدد (". count($cards_amount) . ") تحتاج الى شحن ، الرجاء قم بتعبئة رصيد لتلك البطاقات";
			$status = 1;
		}else{
			$res = "";
			$status = 0;
